...
\texttt{-O3 -march=native -ffast-math -std=c11}. As duas vers\~oes ligam
\texttt{-fopenmp} para medir com \texttt{omp\_get\_wtime()}; a sequencial
acrescenta \texttt{-DSEQ\_ONLY} e n\~ao abre regi\~ao paralela -- $T_1$ n\~ao \'e
a vers\~ao OpenMP com uma thread. \texttt{omp\_get\_num\_procs()} devolve 12.
Medimos a fase computacional completa (c\^amera, aloca\c{c}\~ao, la\c{c}o e
checksum); escrita PPM e print CSV ficam fora, nos dois bin\'arios. Cada ponto
teve 10 execu\c{c}\~oes em m\'aquina ociosa; reportamos mediana, m\'inimo e
m\'aximo. Threads fixadas por \texttt{OMP\_NUM\_THREADS}. O checksum RGB \'e
id\^entico de 1 a 12 threads e em todas as pol\'iticas.
...
Os pragmas est\~ao em \texttt{src/smallpt.c:315--327}:
...
\captionof{table}{Escalabilidade forte ($w{=}800$, $h{=}600$, $\mathrm{spp}{=}32$),
  10 execu\c{c}\~oes. $T_1$ \'e o sequencial com \texttt{-DSEQ\_ONLY}.}
\label{tab:strong}
